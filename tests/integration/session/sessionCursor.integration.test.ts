...
import type { SessionCursor } from '@/session/SessionCursor.js';
...
test('SessionCursor: extractCursor with small result set (≤50 rows)', () => {
  ...
  const filter = { operator: '=', field: 'status', value: 'open' };
...
test('buildWhereFromCursor: with sourceFilter', () => {
  const cursor: SessionCursor = {
    ...
    primaryKeys: ['id'],
    ids: null,
    sourceFilter: { operator: '=', field: 'status', value: 'open' },
    rowCount: 100,
    pipelineId: 'pipeline-123',
    description: 'Test query',
    expiresAt: new Date(Date.now() + 300000),
...
test('buildCursorSystemPromptFragment: includes all cursor details', () => {
  ...
  assert.ok(fragment.includes('use the above context to resolve the target.'));
});
...
test('Integration: Two-turn interaction with cursor-driven write', async () => {
  ...
  const queryFilter = { operator: '=', field: 'status', value: 'open', table: 'tickets' };
  ...
  assert.strictEqual(clause, '"id" = $1');
  ...
  assert.ok(queries[0].sql.includes('"id" = $1'));
...
test('Edge case: Large result uses sourceFilter instead of IDs', () => {
  ...
  const filter = { operator: '=', field: 'status', value: 'open', table: 'tickets' };
...
test('Edge case: Confirm prompt warning for large cursor-driven updates', () => {
  ...
  const largeCursor: SessionCursor = {
    ...
    primaryKeys: ['id'],
    ids: null,
    sourceFilter: { operator: '=', field: 'status', value: 'open', table: 'tickets' },
    rowCount: 847,
    pipelineId: 'pipeline-123',
    description: 'Show all open tickets',
    expiresAt: new Date(Date.now() + 300000),
