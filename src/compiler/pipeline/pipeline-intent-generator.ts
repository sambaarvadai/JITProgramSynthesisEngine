...
export type PipelineIntentGeneratorConfig = {
  ...
  model?: string;
  schema?: SchemaConfig;
  builtSchema?: BuiltSchema; // Full schema with traits
  ...
  async generate(
    ...
    if (isReferentialQuery(naturalLanguageDescription) && this.config.sessionCursorStore) {
      ...
      if (cursor) {
        ...
        systemPrompt += '\n\nThe user appears to be referencing a prior result but no valid session context is available. Ask the user to clarify which record(s) they mean. IMPORTANT: Ask only a direct, concise question without any internal reasoning or explanation. For example: "Which contact are you referring to?" or "Please specify the contact name or ID."';
    ...
    // Filter out auto-injected fields from params using schema traits (generic post-processing)
    // These fields are automatically resolved by the system and should not be treated as user parameters
    if (intent.params && this.config.builtSchema) {
      console.log('[PipelineIntentGenerator] Filtering params using builtSchema traits');
      console.log('[PipelineIntentGenerator] BuiltSchema traits available:', this.config.builtSchema.traits.size);
      console.log('[PipelineIntentGenerator] Params before filtering:', Object.keys(intent.params));

      const autoInjectedTraits = new Set([
        'session_scoped',     // auto-injected from session (workspace_id, etc.)
        'system_audited',     // auto-injected from session user (created_by_user_id, etc.)
        'server_generated',   // auto-generated by database (id, etc.)
        'immutable_after_insert', // cannot be set after insert
        'soft_delete_marker', // managed by soft delete logic
        'computed_from_children' // computed from child records
      ]);

      for (const param of Object.keys(intent.params)) {
        // Check if this param exists in any table with an auto-injected trait
        let shouldExclude = false;
        for (const [tableName, tableTraits] of this.config.builtSchema.traits) {
          if (tableTraits.has(param)) {
            const trait = tableTraits.get(param);
            if (trait && autoInjectedTraits.has(trait.trait)) {
              console.log(`[PipelineIntentGenerator] Excluding param '${param}' with trait '${trait.trait}'`);
              shouldExclude = true;
              break;
            }
          }
        ...
        if (shouldExclude) {
          delete intent.params[param];
        }
      }

      console.log('[PipelineIntentGenerator] Params after filtering:', Object.keys(intent.params));
    } else {
      console.log('[PipelineIntentGenerator] Skipping param filtering - builtSchema not available');
    }
...
7. CRITICAL: When generating params and fields, ALWAYS use the EXACT column names
   from the provided schema. Do NOT generate human-readable names just to make it understandable. Use the exact column names as they
   appear in the schema. This is essential for the generated pipeline to work correctly.
